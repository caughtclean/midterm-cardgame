...
var Deck = (function () {
  ...
  function SuitName(suit) {
    ...
    return suit === 0 ? 'spades' : suit === 1 ? 'clubs' : suit === 2 ? 'diamonds' : 'joker';
  }
  ...
  var poker = {
    deck: function deck(_deck4) {
      ...
      function poker(next) {
        ...
        var player1 = []
        __fontSize = fontSize();
        var reversedSet = cards.slice(-26);

        reversedSet.forEach(function (card, i) {
          if (i < 13) {
            game.player2.cards.push(card)
          }
          if (i >= 13 && i < 26) {


            game.player1.cards.push(card)
          } else {
            game.prize.cards.push(card)
          }
    ...
    card: function card(_card4) {
      ...
      _card4.poker = function (i, len, cb) {
        ...
        switch(_card4.suit){
          case 0:
            x = Math.round((i - 15.05) * 50 * __fontSize / 16);
            y = Math.round(110 * __fontSize / 16);

            break;

          case 1:
            x = Math.round((i - 9.05) * 50 * __fontSize / 16);
            y = Math.round(-110 * __fontSize / 16);
            break;

            case 2:
            x = Math.round((i - 6.05) * 0 * __fontSize / 16);
  ...
  function Deck(jokers) {
    // init cards array
    var cards = new Array(jokers ? 39 : 39);
